    ...
    public class XmlTool
    {
        ...
        public static byte[] SerializeToBinary(object obj)
        {
            using (MemoryStream SerializeStream = new MemoryStream())
            {
                ...
                byte[] data = SerializeStream.ToArray();
                return data;
            }
        }

        private static object DeserializeWithBinary(byte[] data, int offset, int length)
        {
            using (MemoryStream DeserializeStream = new MemoryStream())
            {
                DeserializeStream.Write(data, offset, length);
                DeserializeStream.Position = 0;
                BinaryFormatter bf = new BinaryFormatter();
                object obj = bf.Deserialize(DeserializeStream);
                return obj;
            }
        }
        ...
        /// <returns></returns>
        public static T DeserializeWithBinary<T>(byte[] data, int offset, int length)
        {
            return (T)DeserializeWithBinary(data, offset, length);
        }
